...
function removeOldweather() {
    ...
    for (var i = 0; i < localStorage.length; i++) {
        ...
        if (localStorage.key(i).indexOf(str) > -1) {
            ...
            if ((timenow - wepoch) > 9000) {
                console.log("Removing " + localStorage.key(i) + "</br >");
                remw++;
                // }
                //check that map w is not in weatherdata ... 
                var country = parsed_json['location']['country_name'];
                var location = parsed_json['location']['city'];
        ...
        $.each(fdatap.people, function (i, p) {
            ...
            if (komdata != null) {
                ...
                $.each(komdatap.segs, function (i, fr) {
                    ...
                    //  console.log(p.ID + " " + segID + " " + amount1);
                });
            }
        });
        return total;
...
function calcStarStorage() {
    ...
    if (sdata == null) {
        ...
        $.each(sdatap.segs, function (i, s) {
            ...
            var amount3 = (array.length * 2) / 1024;
            total += amount3;
            //  console.log(s.ID + " " + segID + " " + amount1);
        });
        return total;
    }
}
function calcStorage() {
    ...
    for (var i = 0; i < localStorage.length; i++) {
        if (localStorage.key(i).indexOf(str) > -1) {
            ...
            if ((wdata1 == null) && (fav == null) && (isStarred == false) && (isMap == false)) {
                ...
                unused += amount2;
                //var amount3 = (localStorage.getItem(key3).length * 2) / 1024;
                //unused += amount3;
            }
            if (wdata1 != null) {
                //   live weather
    ...
    //  console.log(calcStarStorage() + " " + star + " " + favc)
    ...
    if (type == "unused") {
        ...
        for (var i = 0; i < localStorage.length; i++) {
            if (localStorage.key(i).indexOf(str) > -1) {
                ...
                if ((wdata1 == null) && (wdata2 == null) && (fav == null) && (isStarred == false) && (isMap == false)) {
                    ...
                    console.log("rem: " + ID + "_weather_map");
                } else {
                    //    alert("not removed " + ID + fav + isStarred + isMap);
                }
            }
        ...
        for (var i = 0; i < localStorage.length; i++) {
            if ((localStorage.key(i) != 'userdata') && (localStorage.key(i) != 'fulluserdata') && (localStorage.key(i) != 'sub') && (localStorage.key(i) != 'starsdata') && (localStorage.key(i) != 'credits') && (localStorage.key(i) != 'oauthio_cache') && (localStorage.key(i) != 'oauthio_provider_strava') && (localStorage.key(i) != 'starsdata')) { //and more todo
                localStorage.removeItem(localStorage.key(i));
                console.log("rem all: " + localStorage.key(i));
            }
        }
        ...
        if ((localStorage.key(i).indexOf(SegID) > -1))
            if (lb == true) {
                ...
                if (localStorage.key(i).indexOf("_hist_") > -1) {
                    console.log("found own hist");
                    if ((localStorage.key(i).split("_").length - 1) == 3) {
                        console.log("count own hist");
                        j++
                    }
                }
                ...
                // couunt frID_segID
                if (localStorage.key(i).indexOf("_hist_") > -1) {
                    console.log("found fr hist");
                    if ((localStorage.key(i).split("_").length - 1) == 4) {
                        console.log("count friend hist");
                        j++
                    }
                }
            }
        //&& (localStorage.key(i).indexOf(SegID) > -1)) { //separate
                            ...
                            function showdata() {
                                ...
                                if (lb == true) {
                                    ...
                                    if (countHistW(count, SegID, frID, lb) == 1) {
                                        ...
                                        if (frID == null) {
                                            while (j < 20) {
                                                console.log("Removing " + SegID + "_" + j + "_hist_user");
                                                // }
                                            ...
                                            while (j < 20) {
                                                console.log("Removing " + SegID + "_" + frID + "_" + j + "_hist_user");
                                                // }
...
function weatherMap1(type, ct) {          //  create a loop function
    var json = localStorage.getItem('seg_loc_data');
    var j2 = eval('(' + json + ')');

    var ID = checkLocalW(); //does seg loc have any wind data?
    console.log("wct=" + ID);
    //if yes, copy to the rest
    if (ID != null) {
        copyMapWeather(ID);
    } else {
        weatherMap(ct);
...
function weatherMap(ct) {          //  create a loop function
    ...
    console.log(j2.points[0].endlatlong + " " + j2.points[0].PID);
